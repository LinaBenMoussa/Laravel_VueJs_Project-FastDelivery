...
<div className="cart-container">
      <h2>Shopping Cart</h2>
      <div v-if="$store.state.Foodstore.cart.length == 0">
        ...
        <div v-else>
          ...
          <div class="cart-items">
            <div v-for="(c) of $store.state.Foodstore.cart" :key="c.product.id">
                <div class="cart-item" key={cartItem.id}>
                  ...
                  <div class="cart-product-price"> {{ c.product.price }} TL</div>
                  <div class="cart-product-quantity">
                    <button @click="minus(c)" >
                      -	
                    </button>
          ...
          <div class="cart-summary">
            ...
            <div class="cart-checkout">
              <div class="subtotal">
                <span>Subtotal</span>
                <span class="amount">{{$store.state.Foodstore.cartTotal}} TL</span>
              </div>
...
onMounted(() => {
       ...
       console.log(store.state.Foodstore.cart)
       getProduits();
           }
);
 ...
 const  removeFromCart=(item)=> { 
        store.commit("Foodstore/removeFromCart", item);
       }

 const  plus=(item)=> { 
         Produits.value.map((pro)=>{
         if(pro.id==item.product.id){
            store.commit("Foodstore/plusCart", item);  
         }       
         });        
       }
 const minus=(item)=> { 
            if(item.qty >1 )
           {  store.commit("Foodstore/minusCart", item); }
         }       
